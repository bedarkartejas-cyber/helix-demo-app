...
export default function ProductDetail() {
    ...
    const [StaticComponent, setStaticComponent] =
    ...
    */
    ...
    useEffect(() => {
        ...
        const loadStaticPage = async () => {
            try {
                // Dynamically load the static page based on the ID
                const module = await import(`./pdp/${id}/index.tsx`);
                setStaticComponent(() => module.default);
            } catch (err) {
    ...
    if (isLoading) {
        return (
            <div className="h-screen flex items-center justify-center bg-slate-900">
                <div className="text-2xl text-white">Loading product...</div>
            </div>
        );
    }
    ...
    // If no PDP sections exist, show a fallback message
    if (!product.pdp_sections || product.pdp_sections.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center h-screen bg-slate-900 text-white">
                <div className="text-2xl mb-4">
...
//         (a, b) => b.is_hero_media - a.is_hero_media
//     );
...
//                                                                 )
//                                                             }
//                                                             className="absolute -top-2 -right-2 text-slate-300 hover:text-red-500 font-bold p-1"
//                                                         >
//                                                             ✕
//                                                         </button>
//                                                     )}
...
//                                                                             )
//                                                                         }
//                                                                         className="p-2 hover:bg-blue-50 cursor-pointer border-b flex items-center gap-2"
//                                                                     >
...
//                                         }
//                                     >
//                                         <td className="p-4 border-b border-r border-slate-200 font-semibold text-slate-600 sticky left-0 bg-inherit z-10">
//                                             {spec}
//                                         </td>
//                                         {compareSlots.map((slot, idx) => (
//                                             <td
//                                                 key={idx}
//                                                 className={`p-4 border-b border-r border-slate-200 text-slate-800 ${idx === 0 ? "font-medium" : ""}`}
//                                             >
